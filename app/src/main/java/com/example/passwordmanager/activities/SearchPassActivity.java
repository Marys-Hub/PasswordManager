...
//                        String n = doc.getString("github");
                            ...
                            assert mapWithSitesAndPass != null;
//                            List<String> sitesName = new ArrayList<>(mapWithSitesAndPass.keySet());
//                            set_up_sites(sitesName,mapWithSitesAndPass);
//                            List<String> nume = new ArrayList<>(mapWithSitesAndPass.keySet());
//                            List<Object> nr = mapWithSitesAndPass.
...
//                           List<String> list_of_sites =  mapWithSitesAndPass.keySet().stream().map(a->a.toString()).collect(Collectors.toList());
//                            sites.add(new Site(nr.get(0), ))
    ...
    private void set_up_sites(List<String> names, Map<String, Object> mapWithSitesAndPass) {
        int idx;
        for (idx = 0; idx < names.size(); idx++) {
            sites.add(new Site(names.get(idx), Objects.requireNonNull(mapWithSitesAndPass.get(names.get(idx))).toString()));
        }
    }
